...
handle_zoom_in(float x, float y, void (*drawscreen) (void)) {
    ...
    //if (find_zoom_level_by_height() > zoom_level_1) {
...
void draw_surface(const Surface& surface, double x, double y) {
    if (surface.getSurface() != NULL) {
        cairo_set_source_surface(x11_state.ctx, surface.getSurface(),
                xworld_to_scrn(x) - 15, yworld_to_scrn(y) - 25);
...
void draw_surface_intersection(const Surface& surface, double x, double y) {
    if (surface.getSurface() != NULL) {
        cairo_set_source_surface(x11_state.ctx, surface.getSurface(),
                xworld_to_scrn(x) - 16, yworld_to_scrn(y) - 35);
        cairo_paint(x11_state.ctx);
    } else {
        cerr << "Surface was not initialized" << endl;
    }
...
void draw_surface_without_offset(const Surface& surface, double x, double y) {
    if (surface.getSurface() != NULL) {
        cairo_set_source_surface(x11_state.ctx, surface.getSurface(),
